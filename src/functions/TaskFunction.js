...
function onSubmitHandlerNewTask(newTaskDetail)
{
    ...
    for( let i=0;i<ObjectKeys.length;i++)
    {
        ...
        else if(!(newTaskDetail["Title"].length>=3) || !(newTaskDetail["Title"].length<=20))
        {
            ErrorMessage="Title must be between 3 and 20 words";
            break;
        }
        else if(!(newTaskDetail["Description"].length>=5) || !(newTaskDetail["Description"].length<=200))
        {
            ErrorMessage="Description must be between 5 and 200 words";
            break;
        }
    }
